...
class ServiceClient(Node):
    def __init__(self):
        super().__init__('service_client')# service_clinet 노드 초기화
        self.client = self.create_client(AddTwoInts, 'add_two_ints')  
        # 'add_two_ints'라는 Service을 사용하는 클라이언트 생성
        while not self.client.wait_for_service(timeout_sec=1.0):
            #service server가 준비 될때 까지 기다림
...
def main(args=None):
    ...
    node = ServiceClient()#service client 노드 생성
